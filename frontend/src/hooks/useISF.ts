...
/**
 * calculateSemaphore
 * Si se pasa `actualMonthSpent` (suma de transacciones de gasto del mes actual),
 * el semáforo refleja el gasto real vs el ingreso mensual.
 * Si no se pasa, usa los campos de presupuesto del perfil (comportamiento clásico).
 */
export function calculateSemaphore(profile: Profile, actualMonthSpent?: number): BudgetSemaphore {
  const {
    monthly_income: income,
    fixed_expenses,
    variable_expenses,
  ...
  const committed = actualMonthSpent !== undefined
    ? actualMonthSpent
    : fixed_expenses + variable_expenses + leisure_expenses + monthly_debt_payment
...
/** Hook: calcula ISF + semáforo reactivo al gasto real del mes */
export function useISF(profile: Profile | null, actualMonthSpent?: number) {
  return useMemo(() => {
    if (!profile) return null
    return {
      breakdown: calculateISF(profile),
      semaphore: calculateSemaphore(profile, actualMonthSpent),
    }
  }, [profile, actualMonthSpent])
}
